...
if uploaded_file:
    ...
    st.info("An ensemble model is used with GridSearchCV for hyperparameter tuning and multi-threaded training for faster processing. "
            "Model metrics are stored so that your custom input selection doesn't reset the trained model.")
    ...
    if target_col:
        ...
        # Train the model only if not already in session state or if target changed
        ...
        # -------------------------------
        # Prediction with a Single Custom Input Field
        # -------------------------------
        st.write("### Make a Prediction with a Custom Input for a Selected Feature")
        st.info("Select the feature for which you want to supply custom input data. All other features will use default values (mean for numeric, mode for categorical).")

        # Dropdown to select which feature to provide custom data for.
        selected_feature = st.selectbox("Select Feature to Customize", options=list(X.columns), key="custom_feature_select")

        # Provide an input widget for the custom value for the selected feature.
        # The widget type depends on the feature type.
        if selected_feature:
            if X[selected_feature].dtype in ['int64', 'float64']:
                custom_value = st.number_input(f"Enter custom numeric value for '{selected_feature}'", 
                                            value=float(X[selected_feature].mean()),
                                            key="custom_value_input")
            else:
                default_val = str(X[selected_feature].mode()[0])
                custom_value = st.text_input(f"Enter custom value for '{selected_feature}'", 
                                            value=default_val,
                                            key="custom_value_input")
        else:
            custom_value = None

        # Build the input vector: use default aggregated value for all features except the one with custom input.
        input_vector = {}
        ...
        # Predict on the constructed input vector.
