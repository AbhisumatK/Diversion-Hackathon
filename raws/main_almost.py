...
with st.sidebar:
    ...
    if uploaded_file:
        ...
        # Select target column and auto-select candidate features
        ...
        # Auto-select features with correlation > 0.1 (absolute)
...
# If the target has 10 or fewer unique values, treat it as a classification task.
is_classification = y.nunique() <= 10
st.session_state.is_classification = is_classification

# Scaling and Polynomial Feature Engineering (if enabled)
...
# Display the cleaned dataset used for training
st.write("### Final Cleaned Dataset for Training")
st.dataframe(df)

# Define function to train a candidate model with hyperparameter search
...
# Candidate models and their hyperparameter grids based on problem type
...
# Build ensemble model using the tuned candidate models
if not is_classification:
...
st.session_state.best_model_name = "Voting Ensemble"
...
for feature in st.session_state.chosen_features:
    # Use the median of the training data as the default value
    default_val = float(df[feature].median())
    input_data[feature] = st.number_input(f"Enter {feature}", value=default_val)

if st.button("Predict"):
    new_data = pd.DataFrame([input_data])
    # Apply polynomial transformation if enabled
    if apply_poly:
