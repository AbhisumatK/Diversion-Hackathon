...
# If a new file is uploaded or the file is changed, clear the stored model and related keys.
if uploaded_file:
    # Compare new file name with stored file name.
    ...
    # Callback to reset stored model when target is changed.
    def reset_model():
        st.session_state.pop("trained_model", None)
        st.session_state.pop("target_col", None)
        st.session_state.pop("input_values", None)
    ...
    if len(num_cols) == 0:
        ...
        if selected_feature:
            # Callback to update numeric input value.
            def update_numeric():
