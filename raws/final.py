...
if uploaded_file:
    ...
    # -------------------------------
# Prediction Model Extension with Single Custom Input Field via Dropdown/Slider
# -------------------------------
    # -------------------------------
# Prediction Model Extension with Single Custom Input Field via Manual Input (No Min/Max)
# -------------------------------
    # -------------------------------
# Prediction Model Extension with Debug Information
# -------------------------------
    st.write("### Prediction Model")
    st.info("An ensemble model (RandomForest) is trained with GridSearchCV and multi-threading. "
            "The model and its metrics are preserved in session state. You can customize one feature manually; "
            "all other features use default aggregated values.")
    ...
    if target_col:
        ...
        # Debug: Show feature importances if available
        if hasattr(best_model, "feature_importances_"):
            st.write("Feature Importances:", dict(zip(X.columns, best_model.feature_importances_)))

        # -------------------------------
        # Prediction with a Single Custom Input for One Selected Feature
        # -------------------------------
        st.write("### Make a Prediction with Custom Input for a Selected Feature")
        st.info("Select the feature you want to override with custom input. All other features use default aggregated values "
                "(median for numeric, mode for categorical).")

        # Dropdown to select which feature to override
        selected_feature = st.selectbox("Select Feature to Customize", options=list(X.columns), key="custom_feature_select")

        # Provide a manual input for the selected feature (free-form number input for numeric, dropdown for categorical)
        if selected_feature:
            if X[selected_feature].dtype in ['int64', 'float64']:
                custom_value = st.number_input(
                    f"Enter custom numeric value for '{selected_feature}'",
                    value=float(X[selected_feature].median()),
                    key="custom_value_input"
                )
            else:
        ...
        # Build the input vector: use the custom value for the selected feature, defaults for others.
        input_vector = {}
        for col in X.columns:
            if col == selected_feature:
                input_vector[col] = custom_value
            else:
                if X[col].dtype in ['int64', 'float64']:
                    input_vector[col] = float(X[col].median())
                else:
                    input_vector[col] = str(X[col].mode()[0])

        # Debug: Show the constructed input vector
        st.write("Input Vector for Prediction:", input_vector)

        # When the Predict button is clicked, use the stored model to predict.
