...
if uploaded_file:
    ...
    # Encode categorical variables for visualization (the prediction pipeline will use OneHotEncoder)
    label_encoders = {}
    ...
    st.info("This advanced prediction model uses a stacking ensemble with optimized preprocessing. "
            ...
            "For dynamic input: if all predictors are categorical, you'll be prompted to provide input for every feature; "
            "otherwise, you'll use the previous dropdown system to modify one feature at a time.")

    # AUTOMATED TARGET SELECTION:
    # If a column named 'target' exists, use it; otherwise, assume the last column as the target.
    ...
    # Prepare predictors (X) and target (y) from the cleaned dataset
    X = df.drop(target_col, axis=1)
    y = df[target_col]

    # Store the training columns once to ensure consistency in feature names.
    ...
    # Build a ColumnTransformer to preprocess features:
    # Numeric features are scaled with StandardScaler and categorical features are mapped using OneHotEncoder.
    transformers = []
    if num_cols:
    ...
    # AUTOMATED MODEL SELECTION: classification if target is object or few unique values; otherwise, regression.
    is_classification = (y.dtype == "object") or (y.nunique() < 10)

    # Train the model if not already stored or if the target variable has changed
    ...
    st.info("If all predictors are categorical, you'll be prompted to provide input for each feature. "
            "Otherwise, select one feature to modify (others use default values: median for numeric, mode for categorical).")

    # Initialize default input values in session state if not already set (using stored training columns)
    ...
    # If all predictors are categorical, prompt for each feature's input.
    if len(num_cols) == 0:
        ...
        # Otherwise, use the previous system: allow the user to select one feature to modify.
