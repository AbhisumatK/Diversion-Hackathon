...
if uploaded_file:
    ...
    # Save encoders for later inverse-mapping predictions.
    label_encoders = {}
    for col in df.select_dtypes(include=['object']).columns:
        le = LabelEncoder()
    ...
    st.info(
        ...
        "You can select the target variable for prediction and override it if desired. "
    ...
    target_col = st.selectbox("Select Target Variable for Prediction", df.columns, index=df.columns.get_loc(default_target))
    ...
    # Reinitialize training columns and input values whenever target changes.
    ...
    # Train the model if not already stored or if the target changed.
    ...
    # For numeric input: update session state once Enter key is pressed.
    if len(num_cols) == 0:
        ...
        if selected_feature:
            # Define a callback that updates the session state when the numeric input changes.
            def update_numeric():
    ...
    # For classification, if the target's original dtype is object, map numeric prediction back to original string.
    if is_classification and (df[target_col].dtype == "object") and (target_col in label_encoders):
