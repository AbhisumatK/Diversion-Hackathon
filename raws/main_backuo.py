...
from sklearn.preprocessing import StandardScaler, LabelEncoder, PolynomialFeatures, FunctionTransformer
...
st.title("Optimized Prediction Model Trainer")

# Initialize session state variables
for key in ["df", "best_model", "scaler", "chosen_features", "is_classification", 
            "best_model_name", "test_score", "poly", "poly_degree", "dataset_type", "target_column"]:
...
with st.sidebar:
    ...
    if uploaded_file:
        ...
        # Candidate features: all columns except the target
        ...
        if not chosen_features and dataset_type == "Numeric":
            # Helper: encode series if needed for correlation computation
            def encode_series(s):
                if s.dtype == 'O':
                    le = LabelEncoder()
...
# Clear everything if no dataset
...
if not st.session_state.chosen_features:
    ...
    if st.session_state.dataset_type == "Textual":
        # Combine selected text columns on the fly without creating a new column
