...
if uploaded_file:
    ...
    # AUTOMATED MODEL SELECTION: use classification if target is object or has few unique values; else regression.
    is_classification = (y.dtype == "object") or (y.nunique() < 10)

    # Train (or retrain) the model if not stored or if target changes.
    ...
    if is_classification and (target_col in label_encoders):
        # Get class probabilities for a confidence measure.
        probs = best_model.predict_proba(input_df)[0]
        confidence = np.max(probs)
        pred_class = best_model.predict(input_df)[0]
        pred_class = label_encoders[target_col].inverse_transform([int(round(pred_class))])[0]
