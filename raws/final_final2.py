...
if uploaded_file:
    ...
    # Encode categorical variables for visualization (save encoders for mapping predictions back)
    label_encoders = {}
    for col in df.select_dtypes(include=['object']).columns:
        le = LabelEncoder()
    ...
    st.info(
        ...
        "You can select the target variable for prediction, and the appropriate model (classification or regression) is used. "
    ...
    # Allow the user to select (or override) the target variable.
    ...
    # If the user changed the target (or it's not stored), re-initialize
    if ("target_col" not in st.session_state) or (st.session_state["target_col"] != target_col):
        # 1) Update training columns
        st.session_state["training_columns"] = list(X.columns)
        # 2) Re-initialize input_values to avoid stale keys
    ...
    # Train the model if not already stored or if the target variable changed.
    ...
    # Build the input vector from st.session_state["input_values"].
    if len(num_cols) == 0:
        ...
        selected_feature = st.selectbox(
            ...
            options=st.session_state["training_columns"],
        ...
        if selected_feature:
            if selected_feature in num_cols:
                ...
                default_index = 0
                if st.session_state["input_values"][selected_feature] in unique_vals:
                    default_index = unique_vals.index(st.session_state["input_values"][selected_feature])
        ...
        # Build the updated input vector
        input_vector = {
            col: st.session_state["input_values"].get(col)
            for col in st.session_state["training_columns"]
        }
    ...
    # If classification and target was originally an object, map back to original strings
    original_target_dtype = df[target_col].dtype
    is_orig_target_categorical = (original_target_dtype == "object")
    if is_classification and is_orig_target_categorical and (target_col in label_encoders):
