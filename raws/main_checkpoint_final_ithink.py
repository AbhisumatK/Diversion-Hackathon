...
if st.session_state.best_model is None:
    ...
    st.write("Select the features for which you want to provide input manually. For the others, default (mean) values will be used.")

    # Wrap prediction inputs in a form to avoid re-running on every selection.
    with st.form("prediction_form"):
        selected_features = st.multiselect("Select features for manual input", st.session_state.chosen_features, key="selected_features")
        prediction_inputs = {}
    ...
    if submitted:
        # Build full input vector ensuring the correct feature order.
        full_input = []
        ...
        # Use the stored transformer pipeline.
