...
st.title("Ez-Viz: ML Visualization & Prediction Tool")
...
# Clear stored keys if a new file is uploaded.
if uploaded_file:
    if "current_file" not in st.session_state or st.session_state["current_file"] != uploaded_file.name:
        ...
        st.session_state.pop("training_columns", None)
    ...
    st.info(
        ...
        "For dynamic input: if all predictors are categorical, you'll provide input for every feature; otherwise, you'll modify one feature at a time."
    )

    # Callback to reset stored model when target changes.
    def reset_model():
        st.session_state.pop("trained_model", None)
        st.session_state.pop("target_col", None)
        st.session_state.pop("input_values", None)
    ...
    # Reinitialize training columns and input values if target changes.
    ...
    # Build ColumnTransformer for preprocessing: scaling + mapping.
    transformers = []
    if num_cols:
    ...
    # AUTOMATED MODEL SELECTION: classification if target is object or has few unique values; else regression.
    is_classification = (y.dtype == "object") or (y.nunique() < 10)

    # To speed up training, we reduce the grid search parameter space and use cv=3.
    if ("trained_model" not in st.session_state) or (st.session_state.get("target_col") != target_col):
        ...
        if is_classification:
            ...
            model = StackingClassifier(
                ...
                final_estimator=LogisticRegression(penalty='l2', C=1.0, solver='lbfgs'),
                n_jobs=-1
            )
    ...
    # Auto-switch output: if classification and target was originally categorical, map numeric prediction to string.
    if is_classification and (target_col in label_encoders):
        probs = best_model.predict_proba(input_df)[0]
        confidence = np.max(probs)
        pred_class = int(round(prediction[0]))
        pred_class = label_encoders[target_col].inverse_transform([pred_class])[0]
        st.write("### Prediction Result:", pred_class, f"(Confidence: {confidence*100:.1f}%)")
    else:
        st.write("### Prediction Result:", prediction[0])

    # ---------------------------------------
    # Model Export Section in Sidebar
    # ---------------------------------------
    st.sidebar.header("Export Trained Model")
    if "trained_model" in st.session_state:
        export_format = st.sidebar.selectbox("Select Export Format", ["pickle", "joblib"], key="export_format")
        if export_format == "pickle":
            import pickle
            model_bytes = pickle.dumps(st.session_state["trained_model"])
            st.sidebar.download_button("Download Model (pickle)", model_bytes, file_name="trained_model.pkl", mime="application/octet-stream")
            ...
            buffer.seek(0)
            st.sidebar.download_button("Download Model (joblib)", buffer, file_name="trained_model.joblib", mime="application/octet-stream")
    else:
        st.sidebar.info("No trained model to export yet.")
